    ...
    char **argv;		/* Argument strings. */
{
	...
	if ((c == 'c') && (argv[2] != NULL) &&
	    ...
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(nodePtr->flags & SELECTED));
	...
	if (argc == 2) {
		...
		Tcl_SetObjResult(interp, Tcl_ObjPrintf("%g %g", fraction, fraction2));
	    }
	} else if (argc == 3) {
	    Node *nodePtr;
	    TagSearch search;
					 ...
					 * up to what's in argc/argv. */
{
	...
	if (nodePtr != NULL)
	    Tcl_SetObjResult(interp, Tcl_NewIntObj(nodePtr->id));
