    ...
    char **argv;		/* Argument strings. */
{
	...
	if (argc == 3) {
		...
		return TCL_ERROR;
	    Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d", gchar));
	} else if (argc == 4) {
	...
	if (argc == 2) {
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(mainPtr->flags & CK_REVERSE_KLUDGE));
	} else if (argc == 3) {
...
#define SETUP(name) \
	...
	SETUP("geometry");
	Tcl_SetObjResult(interp, Tcl_ObjPrintf("%dx%d+%d+%d", winPtr->width,
		winPtr->height, winPtr->x, winPtr->y));
    } else if ((c == 'h') && (strncmp(argv[1], "height", length) == 0)) {
	SETUP("height");
	Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d", winPtr->height));
    } else if ((c == 'i') && (strncmp(argv[1], "ismapped", length) == 0)
	    && (length >= 2)) {
	SETUP("ismapped");
	Tcl_SetObjResult(interp, Tcl_NewBooleanObj(winPtr->flags & CK_MAPPED));
	...
	if (winPtr->parentPtr != NULL)
	    ...
	    && (length >= 4)) {
	SETUP("reqheight");
	Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d", winPtr->reqHeight));
    } else if ((c == 'r') && (strncmp(argv[1], "reqwidth", length) == 0)
	    && (length >= 4)) {
	SETUP("reqwidth");
	Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d", winPtr->reqWidth));
