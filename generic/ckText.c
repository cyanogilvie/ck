    ...
    char **argv;		/* Argument strings. */
{
	...
	if (CkTextCharBbox(textPtr, &index1, &x, &y, &width, &height) == 0) {
	    Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d %d %d %d", x, y, width, height));
	}
	...
	if (argc == 2) {
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(ckBTreeDebug));
	} else {
	...
	if (CkTextDLineInfo(textPtr, &index1, &x, &y, &width, &height, &base)
		== 0) {
	    Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d %d %d %d %d",
	...
	CkTextPrintIndex(&index1, Tcl_GetString(Tcl_GetObjResult(interp)));
    ...
    char **argv;		/* Argument strings. */
{
	...
	if (matchChar >= 0) {
	    ...
	    CkTextPrintIndex(&index, Tcl_GetString(Tcl_GetObjResult(interp)));
	    goto done;
	}
