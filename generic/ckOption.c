				 ...
				 * 0 and TK_MAX_PRIO. */
{
	...
	while (*src != ':') {
	    if ((*src == '\0') || (*src == '\n')) {
		Tcl_SetObjResult(interp, Tcl_ObjPrintf("missing colon on line %d",
			lineNum));
		return TCL_ERROR;
	    }
	    if ((src[0] == '\\') && (src[1] == '\n')) {
	...
	*dst = '\0';
	...
	if (*src == '\0') {
	    Tcl_SetObjResult(interp, Tcl_ObjPrintf("missing value on line %d", lineNum));
	    return TCL_ERROR;
	}
	...
	while (*src != '\n') {
	    if (*src == '\0') {
		Tcl_SetObjResult(interp, Tcl_ObjPrintf("missing newline on line %d",
			lineNum));
		return TCL_ERROR;
	    }
	    if ((src[0] == '\\') && (src[1] == '\n')) {
