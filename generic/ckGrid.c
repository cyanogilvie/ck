    ...
    char **argv;		/* Argument strings. */
{
	...
	Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d %d %d %d",prevX,prevY,x - prevX,y - prevY));
	...
	masterPtr = GetGridBag(master);
	if (argc == 3) {
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(masterPtr->flags & DONT_PROPAGATE));
	    return TCL_OK;
	}
	...
	Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d %d", masterPtr->layoutCache->width,
	...
	for (i=4; i<argc; i+=2) {
		...
		if (argc == 5) {
		    ...
		    Tcl_SetObjResult(interp, Tcl_NewDoubleObj(weight));
	...
	Tcl_SetObjResult(interp, Tcl_ObjPrintf("%d %d", i, j));
    } else {
